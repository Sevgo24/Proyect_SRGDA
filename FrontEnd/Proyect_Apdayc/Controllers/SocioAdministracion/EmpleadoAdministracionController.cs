...
{
    public class EmpleadoAdministracionController : Base
    {


        #region Inicializa Variables Tabs
        ...
        public JsonResult ListarDocumento()
        {
            ...
            try
            {
                ...
                if (documentos != null)
                {
                    ...
                    {
                        var pathWeb = System.Web.Configuration.WebConfigurationManager.AppSettings["RutaWebImgEmpleado"];
        ...
        [HttpPost]
        public JsonResult Insertar(DTOUsuarioEmpleado usuario)
        {
            Resultado retorno = new Resultado();
            try
            {
                ...
                UsuarioEmpleado obj = new UsuarioEmpleado();

                obj.BPS_ID = usuario.Codigo;
                obj.OWNER = GlobalVars.Global.OWNER;
                obj.EMP_POS = usuario.Cargo;
                obj.EMP_ENTRY = usuario.FechaIngreso;
                obj.ENDS = usuario.FechaBaja;
                ...
                obj.RedSocial = obtenerRedSocial();
                    ...
                    {
                        var datos = new BLUsuarioEmpleado().Insertar(obj,
            ...
            if (ObservacionesTmp != null)
            {
                ObservacionesTmp.ForEach(x =>
                {
                    datos.Add(new BEObservationGral
                    {
                        ...
                        ENT_ID = Convert.ToInt32(Constantes.ENTIDAD.EMPLEADO),
            ...
            if (ParametrosTmp != null)
            {
                ParametrosTmp.ForEach(x =>
                {
                    datos.Add(new BEParametroGral
                    {
                        ...
                        ENT_ID = Constantes.ENTIDAD.EMPLEADO,
                        LOG_USER_CREAT = UsuarioActual
                    });
                });
            }
        ...
        private List<BEDireccion> obtenerDirecciones()
        {
            ...
            if (DireccionesTmp != null)
            {
                DireccionesTmp.ForEach(x =>
                {
                    ...
                    obj.OWNER = GlobalVars.Global.OWNER;
                    obj.ENT_ID = Constantes.ENTIDAD.EMPLEADO;
        ...
        public JsonResult ObtenerUsuarioEmpleado(decimal codigo)
        {

            Resultado retorno = new Resultado();
            try
            {
                BLUsuarioEmpleado servicioUD = new BLUsuarioEmpleado();
                var usuario = servicioUD.Obtener(codigo, GlobalVars.Global.OWNER, Constantes.ENTIDAD.EMPLEADO);

                if (usuario != null)
                {
                    DTOUsuarioEmpleado usuarioDTO = new DTOUsuarioEmpleado()
                    {
                        Codigo = usuario.BPS_ID,
                        Cargo = usuario.EMP_POS,
                        FechaIngreso = usuario.EMP_ENTRY,
                        FechaBaja = usuario.ENDS,
        ...
        public JsonResult ActualizarEstado(decimal codigo)
        {
            ...
            try
            {
                ...
                var usu = new BLUsuarioEmpleado();
                usu.CambiarEstado(codigo, GlobalVars.Global.OWNER, UsuarioActual);

                var empleado = usu.ObtenerSinRelacion(codigo, GlobalVars.Global.OWNER);
                JsonResult jrDato = null;
                if (empleado != null)
                {
                    DTOUsuarioEmpleado empleadoDTO = new DTOUsuarioEmpleado()
                    {
                        Codigo = empleado.BPS_ID,
                        Cargo = empleado.EMP_POS,
                        FechaIngreso = empleado.EMP_ENTRY,
                        FechaBaja = empleado.ENDS,
                        Activo = !empleado.ENDS.HasValue
                    };
                    jrDato = Json(empleadoDTO, JsonRequestBehavior.AllowGet); ;
                }
                retorno.data = jrDato;
                retorno.result = 1;
