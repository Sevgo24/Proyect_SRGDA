...
{
    public class RecaudadorAdministracionController : Base
    {
        ...
        private const string K_SESSION_HISTORICO = "___DTOHistoricoUD";
        private const string K_SESSION_HISTORICO_DEL = "___DTOHistoricoDELUD";
        private const string K_SESSION_HISTORICO_ACT = "___DTOHistoricoACTUD";
        ...
        List<DTOHistorico> historicos = new List<DTOHistorico>();
        List<DTORedSocial> redsocial = new List<DTORedSocial>();

        private List<DTOHistorico> HistoricosTmpUPDEstado
        {
            get
            {
                return (List<DTOHistorico>)Session[K_SESSION_HISTORICO_ACT];
            }
            set
            {
                Session[K_SESSION_HISTORICO_ACT] = value;
            }
        }
        private List<DTOHistorico> HistoricosTmpDelBD
        {
            get
            {
                return (List<DTOHistorico>)Session[K_SESSION_HISTORICO_DEL];
            }
            set
            {
                Session[K_SESSION_HISTORICO_DEL] = value;
            }
        }
        public List<DTOHistorico> HistoricosTmp
        {
            get
            {
                return (List<DTOHistorico>)Session[K_SESSION_HISTORICO];
            }
            set
            {
                Session[K_SESSION_HISTORICO] = value;
            }
        }
        ...
        public JsonResult ActualizarEstado(decimal codigo)
        {
            ...
            try
            {
                ...
                var usu = new BLUsuarioRecaudador();
            ...
            catch (Exception ex)
            {
                ...
                retorno.result = 0;
                ucLogApp.ucLog.GrabarLogError("SGRDA", UsuarioActual, "obtener recaudador", ex);
        ...
        {
            historicos = HistoricosTmp;
            Resultado retorno = new Resultado();
            try
            {
                StringBuilder shtml = new StringBuilder();
                shtml.Append("<table border=0 width='100%;' class='k-grid k-widget'>");
                shtml.Append("<thead><tr><th class='k-header' >Oficina</th>");
                shtml.Append("<th class='k-header'>Cargo</th>");
                shtml.Append("<th class='k-header'>Fecha de Inicio</th>");
                shtml.Append("<th class='k-header'>Fecha Fin</th></tr></thead>");

                if (historicos != null)
                {
                    foreach (var item in historicos.OrderBy(x => x.FechaInicio))
                    {
                        shtml.Append("<tr class='k-grid-content'>");
                        shtml.AppendFormat("<td >{0}</td>", item.OficinaDesc);
                        shtml.AppendFormat("<td >{0}</td>", item.LevelDesc);
        ...
        public JsonResult ListarDocumento()
        {
            ...
            try
            {
                ...
                if (documentos != null)
                {
                    ...
                    {
                        var pathWeb = GlobalVars.Global.RutaTabDocumentoRecaudadorWeb;// System.Web.Configuration.WebConfigurationManager.AppSettings["RutaWebImgRecaudador"];
        ...
        [HttpPost]
        public JsonResult Insertar(DTORecaudador usuario)
        {
            Resultado retorno = new Resultado();
            try
            {
                ...
                tienePermiso = new SeguridadController().PuedeRegistrarSocioREC(Convert.ToDecimal(Session[Constantes.Sesiones.CodigoOficina]), usuario.Codigo, Convert.ToString(Session[Constantes.Sesiones.CodigoPerfil]));
                if (!tienePermiso)
                {
                ...
                {
                    UsuarioRecaudador obj = new UsuarioRecaudador();
                    ...
                    obj.OFF_ID = usuario.OficinaComercial;
                    obj.CUR_ALPHA = usuario.MonedaRecaudacion;
                    obj.AGR_DATE = usuario.InicioContrato;
                    obj.LAST_SET = usuario.LiquidacionContrato;
                    obj.LAST_EXP = usuario.LiquidacionGastos;
                    obj.LAST_SET_TOT = usuario.LiquidacionComision;
                    obj.ACC_ID = usuario.CuentaCorriente;
                    obj.COLL_LEVEL = usuario.Nivel;
                    ...
                    {
                        var datos = new BLUsuarioRecaudador().Insertar(obj,
            ...
            if (ObservacionesTmp != null)
            {
                ObservacionesTmp.ForEach(x =>
                {
                    datos.Add(new BEObservationGral
                    {
                        ...
                        ENT_ID = Convert.ToInt32(Constantes.ENTIDAD.RECAUDADOR),
            ...
            if (ParametrosTmp != null)
            {
                ParametrosTmp.ForEach(x =>
                {
                    datos.Add(new BEParametroGral
                    {
                        ...
                        ENT_ID = Constantes.ENTIDAD.RECAUDADOR,
                        LOG_USER_CREAT = UsuarioActual
                    });
                });
            }
        ...
        private List<BEDireccion> obtenerDirecciones()
        {
            ...
            if (DireccionesTmp != null)
            {
                DireccionesTmp.ForEach(x =>
                {
                    ...
                    obj.OWNER = GlobalVars.Global.OWNER;
                    obj.ENT_ID = Constantes.ENTIDAD.RECAUDADOR;
        ...
        public JsonResult ObtenerUsuarioRecaudador(decimal codigo)
        {

            Resultado retorno = new Resultado();
            try
            {
                BLUsuarioRecaudador servicioUD = new BLUsuarioRecaudador();
                var usuario = servicioUD.Obtener(codigo, GlobalVars.Global.OWNER, Constantes.ENTIDAD.RECAUDADOR);

                if (usuario != null)
                {
                    DTORecaudador usuarioDTO = new DTORecaudador()
                    {
                        Codigo = usuario.BPS_ID,
                        OficinaComercial = usuario.OFF_ID,
                        MonedaRecaudacion = usuario.CUR_ALPHA,
                        InicioContrato = usuario.AGR_DATE,
                        LiquidacionContrato = usuario.LAST_SET,
                        LiquidacionGastos = usuario.LAST_EXP,
                        LiquidacionComision = usuario.LAST_SET_TOT,
                        CuentaCorriente = usuario.ACC_ID,
                        Nivel = usuario.COLL_LEVEL,
                    ...
                    if (usuario.Historicos != null) 
                    {
                        historicos = new List<DTOHistorico>();
                        if (usuario.Historicos != null) 
                        {
                            usuario.Historicos.ForEach(s =>
                            {
                                historicos.Add(new DTOHistorico
                                {
                                    IdOficina = s.OFF_ID,
                                    OficinaDesc = new BLOffices().OficinaXID(s.OFF_ID).OFF_NAME,
                                    IdLevel = s.LEVEL_ID,
                                    LevelDesc = s.LEVEL_ID != 0 ? new BLREC_COLL_LEVEL().LISTAR_REC_COLL_LEVEL_X_ID(s.LEVEL_ID).DESCRIPTION :"",
                                    FechaInicio = s.START,
                                    FechaFin = s.ENDS
                                });
                            });
                            HistoricosTmp = historicos;
                        }                        
                    }
