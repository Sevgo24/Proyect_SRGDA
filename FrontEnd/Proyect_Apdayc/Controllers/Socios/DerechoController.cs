...
{
    public class DerechoController : Base
    {
        ...
        public JsonResult ListarDireccion()
        {
            ...
            try
            {
                //valida si puede actualiar usuario de oficina logeado
            //    bool tienePermiso=new SeguridadController().PuedeRegistrarSocioUD(Convert.ToDecimal(Session[Constantes.Sesiones.CodigoOficina]), bpsId);
                ...
                shtml.Append("<th class='k-header'>Direccion</th>");
        ...
        public JsonResult ListarDocumento()
        {
            ...
            try
            {
                ...
                if (documentos != null)
                {
                    ...
                    {
                        var pathWeb = GlobalVars.Global.RutaTabDocumentoDerechoWeb;
        ...
        public JsonResult ListarParametro()
        {
            ...
            try
            {
                ...
                shtml.Append("<th class='k-header'>Subtipo</th>");
                ...
                if (parametros != null)
                {
                    foreach (var item in parametros.OrderBy(x => x.Id))
                    {
                        ...
                        shtml.AppendFormat("<td >{0}</td>", item.SubTipoParametroDesc);
        ...
        [HttpPost]
        public JsonResult Insertar(DTOUsuarioDerecho usuario)
        {
            Resultado retorno = new Resultado();
            try
            {
                bool tienePermiso=true;
                tienePermiso = new SeguridadController().PuedeRegistrarSocioUD(Convert.ToDecimal(Session[Constantes.Sesiones.CodigoOficina]), usuario.Codigo, Convert.ToString(Session[Constantes.Sesiones.CodigoPerfil]));
                if (!tienePermiso)
                {
                ...
                {
                    UsuarioDerecho obj = new UsuarioDerecho();
                    #region Setting Data
                    
                    obj.BPS_ID = usuario.Codigo;
                    obj.OWNER = GlobalVars.Global.OWNER;
                    obj.PAY_ID = usuario.TipoPago;
                    obj.BPS_GROUP = usuario.GrupoEmpresarial;

                    if (usuario.GrupoEmpresarial == 0) obj.BPS_GROUP = null;

                    obj.BPS_DIS_PER = usuario.DescuentoPer;
                    obj.BPS_DIS_AMO = usuario.DescuentoMonto;
                    obj.BPS_DIS_REA = usuario.DescuentoMotivo;
                    obj.ACC_ID = usuario.CuentaContable;

                    obj.BPS_PARTIDA = usuario.Partida;
                    obj.BPS_ZONA = usuario.Zona;
                    obj.BPS_SEDE = usuario.Sede;
                    ...
                    List<BEAsociado> listaAsoDel = null;
                    if (AsociadosTmpDelBD != null)
                    {
                        listaAsoDel = new List<BEAsociado>();
                        AsociadosTmpDelBD.ForEach(x => { listaAsoDel.Add(new BEAsociado { SEQUENCE = x.Id }); });
                    }
                    //setting Correo         activar
                    List<BEAsociado> listaAsoUpdEst = null;
                    if (AsociadosTmpUPDEstado != null)
                    {
                        listaAsoUpdEst = new List<BEAsociado>();
                        AsociadosTmpUPDEstado.ForEach(x => { listaAsoUpdEst.Add(new BEAsociado { SEQUENCE = x.Id }); });
                    }
                    //8.setting Redes Sociales eliminar
                    ...
                    {
                        var datos = new BLUsuarioDerecho().Insertar(obj,
            ...
            if (ObservacionesTmp != null)
            {
                ObservacionesTmp.ForEach(x =>
                {
                    datos.Add(new BEObservationGral
                    {
                        ...
                        ENT_ID = Convert.ToInt32(Constantes.ENTIDAD.USUARIODERECHO),
            ...
            if (ParametrosTmp != null)
            {
                ParametrosTmp.ForEach(x =>
                {
                    datos.Add(new BEParametroGral
                    {
                        ...
                        PAR_SUBTYPE = Convert.ToInt32(x.IdSubTipoParametro),
                        PAR_VALUE = x.Descripcion,
                        ENT_ID = Constantes.ENTIDAD.USUARIODERECHO,
                        LOG_USER_CREAT = UsuarioActual
                    });
                });
            }
        ...
        private List<BEDireccion> obtenerDirecciones()
        {
            ...
            if (DireccionesTmp != null)
            {
                ...
                foreach (var x in DireccionesTmp)
	         {
                    var obj = new BEDireccion();
                    obj.ADD_ID = x.EnBD ? x.Id : decimal.Zero;
                    obj.OWNER = GlobalVars.Global.OWNER;
                    obj.ENT_ID = Constantes.ENTIDAD.USUARIODERECHO;
        ...
        private List<BEAsociado> obtenerAsociados()
        {
            List<BEAsociado> datos = new List<BEAsociado>();

            if (AsociadosTmp != null)
            {
                AsociadosTmp.ForEach(x =>
                {
                    datos.Add(new BEAsociado
                    {
                        SEQUENCE = x.EnBD ? x.Id : decimal.Zero,
                        ROL_ID = Convert.ToString(x.RolTipo),
                        BPSA_ID = x.CodigoAsociado,
                        BPS_ID = x.Codigo,
                        OWNER = GlobalVars.Global.OWNER,
                        ENT_ID = Convert.ToInt32(Constantes.ENTIDAD.USUARIODERECHO),
                        LOG_USER_CREAT = UsuarioActual
        ...
        public JsonResult ObtenerUsuarioDerecho(decimal codigo)
        {

            Resultado retorno = new Resultado();
            try
            {
                BLUsuarioDerecho servicioUD = new BLUsuarioDerecho();
                var usuario = servicioUD.Obtener(codigo, GlobalVars.Global.OWNER, Constantes.ENTIDAD.USUARIODERECHO);

                if (usuario != null)
                {
                    DTOUsuarioDerecho usuarioDTO = new DTOUsuarioDerecho()
                    {
                        Codigo = usuario.BPS_ID,
                        GrupoEmpresarial = usuario.BPS_GROUP,
                        //RazonSocial = new BLSocioNegocio().ObtenerDatos(usuario.BPS_GROUP).BPS_NAME,
                        TipoPago = usuario.PAY_ID,
                        CuentaContable = usuario.ACC_ID,
                        DescuentoMonto = usuario.BPS_DIS_AMO,
                        DescuentoPer = usuario.BPS_DIS_PER,
                        DescuentoMotivo = usuario.BPS_DIS_REA,
                        Activo = !usuario.ENDS.HasValue,
                        Partida = usuario.BPS_PARTIDA,
                        Zona = usuario.BPS_ZONA,
                        Sede = usuario.BPS_SEDE                         
                    };
                        ...
                        if (usuario.Parametros != null)
                        {
                            usuario.Parametros.ForEach(s =>
                            {
                                parametros.Add(new DTOParametro
                                {
                                    ...
                                    IdSubTipoParametro = s.PAR_SUBTYPE,
                                    SubTipoParametroDesc = s.PAR_SUBTYPE_DESC,
                                    EnBD = true,
                                    UsuarioCrea = s.LOG_USER_CREAT,
                                    FechaCrea = s.LOG_DATE_CREAT,
                                    UsuarioModifica = s.LOG_USER_UPDATE,
                    ...
                    if (usuario.Asociados != null)
                    {
                        asociados = new List<DTOAsociado>();
                        if (usuario.Asociados != null)
                        {
                            foreach (var s in usuario.Asociados)
                            {
                              var obj=  new DTOAsociado();
                                    ...
                                    var usu = new BLSocioNegocio().ObtenerDatos(s.BPSA_ID, GlobalVars.Global.OWNER);
                                    if (usu != null)
                                    {
                                        var nombres = "";
                            ...
                            //usuario.Asociados.ForEach(s =>
                            //{
                            //    asociados.Add(new DTOAsociado
                            //    {
                            //        Id = s.SEQUENCE,
                            //        RolTipo = Convert.ToInt32(s.ROL_ID),
                            //        RolTipoDesc = s.ROL_DESC,
                            //        Codigo = s.BPS_ID,
                            //        CodigoAsociado = s.BPSA_ID, 
                            //        EnBD = true,
                            //        Activo = s.ENDS.HasValue ? false : true
                            //    });
                            //});
        ...
        //DAVID
        //AÑADIENDO CODIGO PARA OBTENER DUPLICADO DE DIRECCION 
        ...
        public JsonResult ObtieneAsociadoTmp(decimal idDir)
        {
            ...
            catch (Exception ex)
            {
                retorno.message = Constantes.MensajeGenerico.MSG_ERROR_GENERICO;
                retorno.result = 0;
                ucLogApp.ucLog.GrabarLogError("SGRDA", UsuarioActual, "ObtieneAsociadoTmp", ex);
            }
            return Json(retorno, JsonRequestBehavior.AllowGet);
        }
        ...
        //Obtener Licencias Por Socio
        public JsonResult ObtenerLicenciasPorSocio(int skip, int take, int page, int pageSize, string owner, decimal BPSID)
        {
            Resultado retorno = new Resultado();
            try
            {
                var lista = new BLSocioNegocio().ListarLicenciasxBPSID(BPSID, page, pageSize);

                var tot = lista.Select(s => s.TotalVirtual).Take(1).ToList();//obtiene el total virtual

                if (tot.Count == 0)
                {
                    return Json(new BELicencias { ListaLicencias = lista, TotalVirtual = 0 }, JsonRequestBehavior.AllowGet); //enviando la data
                }
                else
                {
        ...
        //obtener Establecimiento por SOcio negocio
        public JsonResult ObtenerEstablecimientosPorSocio(int skip, int take, int page, int pageSize, string owner, decimal BPSID)
        {
            Resultado retorno = new Resultado();
            try
            {
                var lista = new BLSocioNegocio().ListarEstablecimientosxBPSID(BPSID, page, pageSize);

                var tot = lista.Select(s => s.TotalVirtual).Take(1).ToList();//obtiene el total virtual

                if (tot.Count == 0)
                {
                    return Json(new BEEstablecimiento { Establecimiento = lista, TotalVirtual = 0 }, JsonRequestBehavior.AllowGet); //enviando la data
                }
                else
                {
