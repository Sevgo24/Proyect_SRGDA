...
using System.IO;
using System.Net;
using static Proyect_Apdayc.Clases.Factura_Electronica.Sunat;
...
namespace Proyect_Apdayc.Controllers.Socios
{
    public class SocioController : Base
    {
        ...
        private class Variables
        {
            ...
            public const int SOLICITA_APROBA = 2;
            public const string MSG_NO_PUEDE_MODIFICAR_SOCIO = "No puede modificarse el Socio seleccionado , por favor de comunicarse con el area encargada y entregar los datos para la modificacion ";
            public const string MSG_BORRAR_MEMORIA_CACHE = "Por favor de eliminar la memoria cache  y reiniciar el navegador  | Si el error continua por favor de comunicarse con el Administrador del Modulo";
        ...
        public JsonResult ListarDireccion()
        {
            ...
            try
            {
                if (!isLogout(ref retorno))
                {
                    ...
                    shtml.Append("<thead><tr><th class='k-header' >Id</th><th  class='k-header'>Tipo Dirección</th>");
        ...
        public JsonResult ListarParametro()
        {
            ...
            try
            {
                if (!isLogout(ref retorno))
                {
                    StringBuilder shtml = new StringBuilder();
                    shtml.Append("<table border=0 width='100%;' class='k-grid k-widget'>");
                    shtml.Append("<thead><tr><th class='k-header' >Id</th><th  class='k-header'>Tipo</th>");
                    shtml.Append("<th class='k-header'>Sub Tipo</th>");
            ...
            try
            {
                if (!isLogout(ref retorno))
                {
                    if (socio.valida == Variables.DOS)
                    {
                        ...
                        var ValidaModifSOcio = new BLSocioNegocio().ValidaPermisoModificacionSocion();
                        ...
                        else
                        {
                            ...
                            if (resultado)
                            {
                                ...
                                else if (socio.Codigo > 0 && (opcAdm == Variables.SI || ValidaModifSOcio == Variables.NO)) // SI ES DIFERENTE A 0 ES POR QUE SE ESTA MODIFICANDO  Y TIENE QUE TENER PERMISOS DE ADMINSITRADOR DE OFF
                                {
                                ...
                                {
                                    retorno.result = Variables.SOLICITA_APROBA;
                                    retorno.message = Variables.MSG_NO_PUEDE_MODIFICAR_SOCIO;
                    ...
                    else
                    {
                        ...
                        retorno.message = Variables.MSG_BORRAR_MEMORIA_CACHE;
                    }
                }
            }
            catch (Exception ex)
        ...
        public JsonResult ValidaRUCSunat(string RUC)
        {
            Sunat su = new Sunat();
            Resultado retorno = new Resultado();
            Myclass2 Datos2 = null;

            try
            {
                var Datos = su.ValidarRUC_Sunat(RUC);
                Datos2 = su.ValidarRUC_Sunat2(RUC);
                Datos.UBIGEO = Datos2.ubigeo;
                ...
                //retorno.valor = Datos.razon_social;
                //retorno.valor2 = Datos.nombre_comercial;
            }
            catch (Exception ex)
            {
        ...
        public JsonResult ValidarDNI_Sunat(string DNI)
        {
            Sunat su = new Sunat();
            Resultado retorno = new Resultado();
            var Datos = su.ValidarDNI_Sunat(DNI);
            try
            {
                retorno.message = "DNI VALIDO.";
                retorno.result = 1;
                retorno.valor = Datos.Nombre_Completo;
                retorno.valor2 = Datos.Apellido_Paterno;
                retorno.valor3 = Datos.Apellido_Materno;
            }
            catch (Exception ex)
            {
                var mensaje = ex.Message;
        ...
        public JsonResult ValidaObligatoriosRegistroSocio()
        {
            Resultado retorno = new Resultado();

            if ((TelefonosTmp != null) && (CorreosTmp != null))
            {
                if (TelefonosTmp.Count > Variables.NO && CorreosTmp.Count > Variables.NO)
                {
                    retorno.result = Variables.SI;
                    retorno.message = "";
                }
