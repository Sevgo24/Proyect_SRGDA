$(function () {
    ...
    $("#btnNuevo").on("click", function () {
        location.href = "../ComisionProducto/Nuevo";
    });
...
function ObtenerTarifaTemporalidad(idMod, idTemp) {
    $.ajax({
        url: "../ComisionProducto/ObtieneTarifaTemporalidad",
        type: 'GET',
    ...
    $("#grid").kendoGrid({
        dataSource: {
            ...
            transport: {
                read: {
                    type: "POST",
                    url: "../ComisionProducto/ListaComisionesProd",
                    dataType: "json"
                },
            ...
            schema: { data: "ListaComisionPro", total: 'TotalVirtual' }
        },
        sortable: true,
        pageable: {
            messages: {
        ...
        columns:
           [
            ...
            {
                hidden: true,
                field: "OWNER", width: 10, title: "<font size=2px>PROPIETARIO</font>", template: "<a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray !important;'>${OWNER}</a>"
            },
            { field: "MOD_ID", width: 50, title: "<font size=2px>Id</font>", template: "<a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray !important;'>${MOD_ID}</a>" },
            { field: "LEVEL_ID", hidden: true, width: 10, title: "Id", template: "<label for='lbls' id='chkSeloff' class='kendo-chk-off' name='chkSeloff' value='${LEVEL_ID}'/>${LEVEL_ID}</label> " },
            { field: "COM_DESC", width: 70, title: "<font size=2px>Tipo comisión</font>", template: "<font color='green'><a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray !important;'>${COM_DESC}</a></font>" },
            { field: "DESCRIPTION", width: 100, title: "<font size=2px>Nivel agente</font>", template: "<font color='green'><a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray !important;'>${DESCRIPTION}</a></font>" },
            { field: "MOD_DEC", width: 120, title: "<font size=2px>Producto</font>", template: "<font color='green'><a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray !important;'>${MOD_DEC}</a></font>" },
            { field: "Formato", width: 40, title: "<font size=2px>Formato</font>", template: "<font color='green'><a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray !important;'>${Formato}</a></font>" },
            { field: "Valor", width: 60, title: "<font size=2px>Valor comisión</font>", template: "<font color='green'><a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray !important;'>${Valor}</a></font>" },
            { field: "COM_START", type: "date", width: 100, title: "<font size=2px>Fecha de vigencia</font>", template: "<a id='single_2' href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray;text-decoration:none;font-size:11px'>" + '#=(COM_START==null)?"":kendo.toString(kendo.parseDate(COM_START,"dd/MM/yyyy"),"dd/MM/yyyy") #' + "</a></font>" },
            { field: "ESTADO", width: 60, title: "<font size=2px>Estado</font>", template: "<a id='single_2'  href=javascript:editar('${MOD_ID}','${LEVEL_ID}') style='color:gray;text-decoration:none;font-size:11px'>${ESTADO}</a>" },
           ]
    });
}
...
function limpiar() {
    ...
    loadTipoComision('ddlTipoComision', 0);
    loadTarifaMantenimiento('ddlTarifa', 0);
    loadTemporalidad('ddlTemporalidad', 0, 0);
}
