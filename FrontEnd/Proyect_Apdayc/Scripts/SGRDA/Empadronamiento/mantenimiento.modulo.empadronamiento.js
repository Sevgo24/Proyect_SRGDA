...
function loadData(e) {
    ...
    var data_sourceLic = new kendo.data.DataSource({
        ...
        transport: {
            read: {
                url: "../ModuloEmpadronamiento/USP_MatrizEmpadronamiento_LISTARPAGEJSON",
                dataType: "json"
            },
...
function loadComboAnio(control, valSel) {
    ...
    $.ajax({
        url: '../ModuloEmpadronamiento/ListarAnios',
        type: 'POST',
...
function loadComboMesXAnio(control, valSel) {
    ...
    $('#' + control).append($("<option />", { value: 0, text: K_ITEM.CHOOSE }));
    $.ajax({
        url: '../ModuloEmpadronamiento/ListarMeses',
        type: 'POST',
        data: { anio: anio },
        beforeSend: function (idDependencia) { },
        success: function (response) {
...
function addRangoComision() {
    ...
    if (idRango != 0) {
        $.ajax({
            url: '../ModuloEmpadronamiento/InsertarRangoComision',
            type: 'POST',
...
function TablaRangoComision() {
    ...
    $.ajax({
        url: '../ModuloEmpadronamiento/Listar_TablaComision',
        type: 'POST',
        data: {},
        beforeSend: function () { },
        success: function (response) {
...
function eliminarRangoComision(id) {
    ...
    Confirmar(' ¿Confirmas eliminar el rango de comision?',
       function () {
           $.ajax({
               url: '../ModuloEmpadronamiento/EliminarRangoComision',
               type: 'POST',
               data: { id: id },
               beforeSend: function () { },
               success: function (response) {
...
function loadComboRango(control, valSel) {
    ...
    $('#' + control).append($("<option />", { value: 0, text: K_ITEM.CHOOSE }));
    $.ajax({
        url: '../ModuloEmpadronamiento/ComboRangoComision',
        type: 'POST',
        beforeSend: function (idDependencia) { },
        success: function (response) {
            var dato = response;
...
function ExportarReportef(tipo) {
    ...
    $("#contenedor").show();
    $.ajax({
        url: '../ModuloEmpadronamiento/ReporteTipo',
        type: 'POST',
        data: {
            anio: anio,
            mes: mes,
        ...
        success: function (response) {
            ...
            if (dato.result == 1) {
                url = "../ModuloEmpadronamiento/ReporteDeEmpadronamiento?" +
      "MES=" + mes_nombre + "&" +
      "ANIO=" + anio + "&" +
      "formato=" + tipo + "&" +
...
function ExportarReportef2(tipo) {
    ...
    //var load = '../Images/otros/loading.GIF';
    //$('#externo').attr("src", load);
    var url = "../ModuloEmpadronamiento/ReporteDeEmpadronamiento?" +
        "MES=" + mes_nombre + "&" +
          "ANIO=" + anio + "&" +
          "formato=" + tipo + "&" +
          "ID_OFICINA=" + idoficina + "&" +
