...
function ExportarReportef(tipo) {
    ...
    if (validafecha == 1) {
        $("#contenedor").show();
        $.ajax({
            url: '../REPORTE_DE_LICENCIAS_NUEVAS/ReporteTipo',
            type: 'POST',
            data: { finicio: ini, ffin: fin, formato: tipo, ID_SOCIO: ID_SOCIO, ID_MODALIDAD: GrupoModalidad, ID_OFICINA: idoficina, Estado: estado },
            beforeSend: function () {
            ...
            success: function (response) {
                ...
                if (dato.result == 1) {
                    url = "../REPORTE_DE_LICENCIAS_NUEVAS/ReporteDeLicenciasNuevas?" +
          "fini=" + ini + "&" +
          "ffin=" + fin + "&" +
          "formato=" + tipo + "&" +
          "ID_SOCIO=" + ID_SOCIO + "&" +
...
function ExportarReportef2(tipo) {
    ...
    var Id_Modalidad = $("#hidModalidad").val() == "" ? 0 : $("#hidModalidad").val();
    var selectedValue = $('#ddtipo').val();
    var url = "../REPORTE_DE_LICENCIAS_NUEVAS/ReporteDeLicenciasNuevas?" +
          "fini=" + ini + "&" +
          "ffin=" + fin + "&" +
          "formato=" + tipo + "&" +
          "ID_SOCIO=" + ID_SOCIO + "&" +
