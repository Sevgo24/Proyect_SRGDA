...
function loadComboTipoCobro(control, valSel) {
    ...
    $.ajax({
        url: '../ReporteDiarioStandar/LISTA_TIPO_COBRO',
        type: 'POST',
...
function ExportarReportef(tipo) {
    ...
    var validaselecciondelCombo = ValidarSeleccionCombo(oficina_id);
    //Si la validacion de fecha es Igual a 1 entonces :
    if (validafecha == 1 && validaselecciondelCombo == 1) {
        $("#contenedor").show();
        //alert(oficina_id)
        url = "http://ws2016-002/ReportServer_SSRS/Pages/ReportViewer.aspx?%2fLyrics%2fDiarioCaja%2fRSDiarioCaja&" +
       "fecha1=" + ini + "&" +
       "fecha2=" + fin + "&" +
...
function ExportarReportef2(tipo) {
    ...
    if (validafecha == 1 && validaselecciondelCombo == 1) {
        ...
        $.ajax({
            url: '../ReporteDiarioStandar/ReporteTipo',
            type: 'POST',
            //data: { fini: ini, ffin: fin, rubro: rubro, idoficina: idoficina, nombreoficina: nombreoficina },
            data: {
            ...
            success: function (response) {
                ...
                if (dato.result == 1) {
                    //var url = "../ReporteDiarioStandar/ReporteDiarioCaja?" + "fini=" + ini + "&" + "ffin=" + fin + "&"
                    //       + "formato=" + tipo + "&" + "rubro=" + rubro + "&" + "idoficina=" + idoficina + "&" + "nombreoficina=" + nombreoficina;
                    url = "../ReporteDiarioStandar/ReporteDiarioCaja?" + "fini=" + ini + "&" + "ffin=" + fin + "&"
                   + "formato=" + tipo + "&" + "rubro=" + rubro + "&" + "idoficina=" + idoficina + "&" + "nombreoficina=" + nombreoficina
...
function BuscarModalidadesXOficina(dato) {
    var soc = dato;
    $.ajax({
        url: '../ReporteDiarioStandar/ConsultaModalidadXOficina',
        type: 'POST',
        data: {
            IdOficina: soc
        },
...
function obtenerModalidadesSeleccionadas() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        type: 'POST',
        url: '../ReporteDiarioStandar/ModalidadesSeleccionadasTemporalesOriginal',
        data: ReglaValor,
        success: function (response) {
            var dato = response;
            if (dato.result == 1) {
