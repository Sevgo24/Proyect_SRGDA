...
$(function () {
    ...
    $('#txtPCuotaFederaria').on(eventoKP, function (e) { return solonumeros(e); });
    $('#txtVCuotaFederaria').on(eventoKP, function (e) { return solonumeros(e); });
    $('#txtPBonificacionAsociacion').on(eventoKP, function (e) { return solonumeros(e); });
    $('#txtVBonificacionAsociacion').on(eventoKP, function (e) { return solonumeros(e); });
    $('#txtFono').on(eventoKP, function (e) { return solonumeros(e); });

    $('#txtFInicioBonificacion,#txtFFinBonificacion,#txtFecha').kendoDatePicker({
        format: "dd/MM/yyyy",
        culture:"es-ES"
    });

    $("#ddlTarifa").on("change", function () {
        msgError("", "ddlTarifa");
    });
...
function ListarDireccionxPerfil(codeEdit) {
    $.ajax({
        url: '../Derecho/ListarDireccionxPerfil',
        type: 'POST',
        data: { id: codeEdit },
        beforeSend: function () { },
...
function insertar(estado) {
    ...
    if (ValidarRequeridos()) {
        ...
        var usuario = {
            Codigo: $("#hidBpsId").val(),
            Tarifa: $("#ddlTarifa").val(),
            InicioBonificacion: $("#txtFInicioBonificacion").val(),
            FinBonificacion: $("#txtFFinBonificacion").val(),
            PCuotaFederativa: $("#txtPCuotaFederaria").val(),
            VCuataFederativa: $("#txtVCuotaFederaria").val(),
            PBonificacionAsociado: $("#txtPBonificacionAsociacion").val(),
            VBonificacionAsociado: $("#txtVBonificacionAsociacion").val(),
    ...
    if (ValidarObligatorio(K_DIV_MESSAGE.DIV_TAB_POPUP_DOCUMENTO, K_DIV_POPUP.DOCUMENTO)) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (dato.result == 1) {
                    if ($("#file_upload").val() != "") {
                        InitUploadTabDocAsociacion("file_upload", dato.Code);
                    }
                    loadDataDocumento();
                } else {
                    alert(dato.message);
                    ...
                    success: function (response) {
                        ...
                        if (dato.result === 1) {
                            ...
                            if (usuario != null) {
                                ...
                                var d1 = $("#txtFInicioBonificacion").data("kendoDatePicker");
                                var d2 = $("#txtFFinBonificacion").data("kendoDatePicker");

                                $("#divTituloPerfil").html("Asociación / Actualización");
                                K_ACCION_ACTUAL = K_ACCION.Modificacion;
                                loadTarifa("ddlTarifa", usuario.Tarifa);

                                var valFechaIni = formatJSONDate(usuario.InicioBonificacion);
                                var valFechaFin = formatJSONDate(usuario.FinBonificacion);

                                d1.value(valFechaIni);
                                d2.value(valFechaFin);

                                $("#txtPCuotaFederaria").val(usuario.PCuotaFederativa);
                                $("#txtVCuotaFederaria").val(usuario.VCuataFederativa);
                                $("#txtPBonificacionAsociacion").val(usuario.PBonificacionAsociado);
                                $("#txtVBonificacionAsociacion").val(usuario.VBonificacionAsociado);
                            ...
                            if (dato.Code == K_TIPO_ERROR.CODE_ERROR_NO_DATA) {
                                $("#btnStatus ,#btnGrabar").button({ disabled: false });
                                $("#divTituloPerfil").html("Asociacion / Nuevo");
                                K_ACCION_ACTUAL = K_ACCION.Nuevo;
                                loadTarifa("ddlTarifa", 0);
                            }
                            else {
                                alert(dato.message);
                            }
