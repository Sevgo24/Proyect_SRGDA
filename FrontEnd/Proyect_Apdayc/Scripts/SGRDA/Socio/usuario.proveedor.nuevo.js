...
$(function () {
    ...
    $("#ddlActividadEcon").on("change", function () {
        msgError("", "ddlActividadEcon");
        msgError("", "txtLineaArticulo");
    });
    ...
    $("#btnRegresar").on("click", function () { location.href = "../Socio/"; }).button();
    //$("#btnNuevo").on("click", function () { location.href = "Nuevo"; });
    $("#btnStatus").on("click", function () {
        limpiarObservacion(); $("#hidEstado").val(1); $("#mvObservacion").dialog("open");
    }).button();
...
function insertar(estado) {
    ...
    msgError("");
     if (ValidarRequeridos() ) {
            ...
            var usuario = {
                Codigo:$("#hidBpsId").val(),
                ActividadEcon: $("#ddlActividadEcon").val(),
                LineaArticulo: $("#txtLineaArticulo").val(),
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (dato.result == 1) {
                        ...
                        //location.href = "../Derecho/Nuevo?set=" + $("#hidBpsId").val();
                    } else {
                        msgError(dato.message);
    ...
    if (ValidarObligatorio(K_DIV_MESSAGE.DIV_TAB_POPUP_DOCUMENTO, K_DIV_POPUP.DOCUMENTO)) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (dato.result == 1) {
                    if ($("#file_upload").val() != "") {
                        InitUploadTabDocProveedor("file_upload", dato.Code);
                    }
                    loadDataDocumento();
                } else {
                    alert(dato.message);
                    ...
                    success: function (response) {
                        ...
                        if (dato.result === 1) {
                            ...
                            if (usuario != null) {
                                ...
                                $("#divTituloPerfil").html("Usuario de Proveedor / Actualización");
                                K_ACCION_ACTUAL = K_ACCION.Modificacion;
                                loadActividadEcon("ddlActividadEcon", usuario.ActividadEcon);
                                $("#txtLineaArticulo").val(usuario.LineaArticulo);
                            ...
                            if (dato.Code == K_TIPO_ERROR.CODE_ERROR_NO_DATA) {
                                $("#btnStatus ,#btnGrabar").button({ disabled: false });
                                $("#divTituloPerfil").html("Usuario de Proveedor / Nuevo");
                                K_ACCION_ACTUAL = K_ACCION.Nuevo;
                                loadActividadEcon("ddlActividadEcon", 0);
                            }
                            else {
                                alert(dato.message);
                            }
