...
$(function () {
    ...
    $("#btnNuevo").on("click", function () {
        document.location.href = '../TarifaPlantilla/Nuevo';
    });
...
function loadData() {
    ...
    $("#grid").kendoGrid({
        dataSource: {
            ...
            transport: {
                read: {
                    url: '../TarifaPlantilla/Listar', dataType: "json",
                    dataType: "json"
                },
            ...
            schema: { data: 'ListarTarifaPlantilla', total: 'TotalVirtual' }
        },
        groupable: false,
        sortable: true,
        pageable: {
        ...
        columns:
			[
				{ title: 'Eliminar', width: 18, template: "<input type='checkbox' id='chkSel' class='kendo-chk' name='chkSel' value='${TEMP_ID}'/>" },
				{ field: "TEMP_ID", width: 20, title: "Id", template: "<a id='single_2'  href=javascript:editar('${TEMP_ID}') style='color:gray;text-decoration:none;'>${TEMP_ID}</a>" },
				{ field: "TEMP_DESC", width: 80, title: "Descripción", template: "<a id='single_2'  href=javascript:editar('${TEMP_ID}') style='color:gray;text-decoration:none;'>${TEMP_DESC}</a>" },
                { field: "TEMP_NVAR", width: 35, title: "N° Caracteristicas", template: "<a id='single_2'  href=javascript:editar('${TEMP_ID}') style='color:gray;text-decoration:none;'>${TEMP_NVAR}</a>" },
                { field: "STARTS", type: "date", width: 30, title: "F. Vigencia", template: "<a id='single_2' href=javascript:editar('${TEMP_ID}') style='color:gray;text-decoration:none;'>" + '#=(STARTS==null)?"":kendo.toString(kendo.parseDate(STARTS,"dd/MM/yyyy"),"dd/MM/yyyy") #' + "</a></font>" },
				{ field: "ESTADO", width: 20, title: "Estado", template: "<a id='single_2'  href=javascript:editar('${TEMP_ID}') style='color:gray;text-decoration:none;'>${ESTADO}</a>" },
			]
    });
}

function editar(idSel) {
    document.location.href = '../TarifaPlantilla/Nuevo?id=' + idSel;
}
