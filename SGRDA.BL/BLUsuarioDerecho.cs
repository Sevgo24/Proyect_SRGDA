...
namespace SGRDA.BL
{
    public class BLUsuarioDerecho
    {
        public int Insertar(SocioNegocio bps)
        {
            ...
            using (TransactionScope transa = new TransactionScope())
            {
                ...
                if (bps.Asociados != null)
                {
                    foreach (var asociado in bps.Asociados)
                    {
                        var result = new DAAsociado().Insertar(asociado);
                    }
                }
        ...
        public int Insertar(UsuarioDerecho bps, 
                                ...
                                List<BEAsociado> asoEliminar, List<BEAsociado> listAsociadoActivar,
                                ...
                                )
        {
            ...
            //{
                ...
                upd = new DAUsuarioDerecho().Insertar(bps);

                var objUsuarioDerecho = new DAUsuarioDerecho().Obtener(bps.BPS_ID, bps.OWNER);
                    ...
                    if (bps.Direcciones != null)
                    {
                        ...
                        foreach (var direccion in bps.Direcciones)
                        {
                            ...
                            else
                            {
                                ...
                                direccion.LOG_USER_UPDATE = bps.LOG_USER_CREAT;
                    ...
                    ///logica de negocio para actualizar asociado
                    DAAsociado proxyAso = new DAAsociado();

                    if (bps.Asociados != null)
                    {
                        foreach (var item in bps.Asociados)
                        {
                            ///verifica si  no existe la Observacion para el socio
                            ///si no existe se registra y asocia la nueva Observacion
                            BEAsociado proxyAsoObtener = proxyAso.ObtenerAsoBPS(bps.OWNER, bps.BPS_ID, item.BPSA_ID);
                            if (proxyAsoObtener == null)
                            {
                    ...
                    if (asoEliminar != null)
                    {
                        asoEliminar.ForEach(x => { proxyAso.Eliminar(bps.OWNER, x.BPS_ID,x.SEQUENCE, bps.LOG_USER_UPDATE); });
                        //foreach (var item in telEliminar)
                        //{
                        //    proxyTel.Eliminar(bps.OWNER, item.PHONE_ID, bps.LOG_USER_UPDATE);
                        //}
                    ...
                    if (listAsociadoActivar != null)
                    {
                        ...
                        //    proxyDoc.Activar(bps.OWNER, item.DOC_ID, bps.LOG_USER_UPDATE);
                        //}
                        listAsociadoActivar.ForEach(x => { proxyAso.Activar(bps.OWNER, x.BPS_ID,x.BPSA_ID, bps.LOG_USER_UPDATE); });
                    }
        ...
        /// <returns></returns>
        public UsuarioDerecho Obtener(decimal codigoBps, string owner,decimal tipoEntidad)
        {
            var objSocio= new DAUsuarioDerecho().Obtener(codigoBps, owner);
            if (objSocio != null) {
                ...
                objSocio.Asociados = new DAAsociado().AsociadoXSocio(objSocio.BPS_ID, objSocio.OWNER);
    ...
    /// <summary>
    /// Cambia el estado del registro de usuario de derecho 
    /// </summary>
    /// <param name="bpsId"></param>
    /// <param name="empresa"></param>
    /// <param name="usuModifica"></param>
    /// <returns></returns>
        public int CambiarEstado(decimal bpsId, string empresa, string usuModifica)
        {
            return new DAUsuarioDerecho().CambiarEstado(bpsId, empresa, usuModifica);
        }

        public UsuarioDerecho ObtenerSinRelacion(decimal codigoBps, string owner)
        {
            var objSocio = new DAUsuarioDerecho().Obtener(codigoBps, owner);
